// src/pages/AdminPages.jsx — AdminClients, AdminWorkers, AdminApprovals, AdminSupport, AdminChats com Firestore granular
import { useState } from 'react';
import { createPortal } from 'react-dom';
import { useAppStore } from '../store';
import Header from '../components/ui/Header';
import { addMessageToSupportTicket, updateSupportTicketStatus } from '../services/supportService';
...
function AdminUserDetailModal({ user, onClose }) {
  ...
  const details =
    user.role === 'CONTRATANTE'
      ? contratanteDetails.find((d) => d.user_id === user.id || d.id === user.id)
      : workerDetails.find((d) => d.user_id === user.id || d.id === user.id);
  const userOrders =
    user.role === 'CONTRATANTE'
      ? orders.filter((o) => o.contratante_id === user.id)
      : orders.filter((o) => o.worker_id === user.id);
      ...
      <div className="bg-white w-full max-w-3xl max-h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-slide-down">
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          <h3 className="font-extrabold text-lg flex items-center gap-2">
            <i className="ph-fill ph-identification-badge text-[#EA1D2C]" /> Dossiê do Usuário
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <div className="flex gap-6 mb-8">
            <img
              src={user.foto_perfil_url || 'https://ui-avatars.com/api/?name=User&background=ea1d2c&color=fff'}
            ...
            <div>
              ...
              <div className="flex gap-2 mt-2">
                ...
                <span className="text-xs font-bold px-3 py-1 rounded-lg bg-gray-100 text-gray-600 border border-gray-200">
                  ID: {user.id}
                </span>
              </div>
              <p className="text-xs text-gray-400 font-bold mt-2">
                Cadastrado: {user.data_criacao ? new Date(user.data_criacao).toLocaleDateString('pt-BR') : '—'}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="bg-gray-50 p-5 rounded-2xl border border-gray-100">
              ...
              <p className="font-bold text-[#1F2937] text-sm mt-1">{user.telefone}</p>
              <p className="font-bold text-[#1F2937] text-sm mt-1">CPF: {details?.cpf || '—'}</p>
            </div>
            <div className="bg-gray-50 p-5 rounded-2xl border border-gray-100">
          ...
          {user.role === 'TRABALHADOR' && (
            <div className="mb-8 bg-blue-50 p-5 rounded-2xl border border-blue-100">
              ...
              <p className="text-sm text-blue-900">
                <strong>Categoria:</strong> {details?.categorias?.join(', ') || 'Geral'}
              </p>
              <p className="text-sm text-blue-900 mt-2">
                <strong>Bio:</strong> {details?.bio || 'Sem apresentação.'}
              </p>
            </div>
          )}
          <div>
            <h4 className="text-sm font-extrabold text-[#1F2937] uppercase mb-4 border-b border-gray-100 pb-2">
              Histórico de Atividade
            </h4>
            <div className="space-y-3 max-h-[200px] overflow-y-auto pr-2">
              {userOrders.length === 0 ? (
                <p className="text-sm text-gray-500 italic bg-gray-50 p-4 rounded-xl">Nenhuma atividade registrada.</p>
              ) : (
                userOrders.map((o) => {
                  const srv = services.find((s) => s.id === o.servico_id);
                  const other = users.find(
                    (u) => u.id === (user.role === 'CONTRATANTE' ? o.worker_id : o.contratante_id)
                  );
                  return (
                    <div
                      key={o.id}
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm font-bold text-[#1F2937] mt-1">{srv?.titulo || 'Serviço Personalizado'}</p>
                        <p className="text-xs text-gray-500">com {other?.nome || 'Usuário'}</p>
                      </div>
                      <div className="font-extrabold text-[#EA1D2C]">R$ {o.preco_final || 0}</div>
                    </div>
                  );
                })
              )}
                ...
                {usersList.length === 0 ? (
                  ...
                  usersList.map((u) => (
                    <tr key={u.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <img
                            src={
                              u.foto_perfil_url ||
                              'https://ui-avatars.com/api/?name=User&background=ea1d2c&color=fff'
                            }
                      ...
                      <td className="p-4 hidden sm:table-cell text-sm font-medium text-gray-600">
                        {u.data_criacao ? new Date(u.data_criacao).toLocaleDateString('pt-BR') : '—'}
                      </td>
                      <td className="p-4 text-right flex items-center justify-end gap-2">
                        <button
                          onClick={() => setSelectedUser(u)}
                          className="bg-gray-100 hover:bg-[#EA1D2C] hover:text-white text-gray-700 font-bold py-2 px-3 md:px-4 rounded-lg text-xs md:text-sm transition shadow-sm"
                        >
                          Ver Dossiê
                        </button>
                        {u.role !== 'ADMIN' && (
                          <button
                            ...
                            className="bg-red-50 hover:bg-red-100 text-red-500 hover:text-red-700 font-bold p-2 md:py-2 md:px-3 rounded-lg text-xs md:text-sm transition border border-red-100"
                            title="Banir Usuário"
                          >
...
export function AdminWorkers() {
  ...
  return (
    <AdminListLayout
      title="Profissionais Aprovados"
      usersList={users.filter((u) => u.role === 'TRABALHADOR' && u.status === 'APPROVED')}
    />
  );
}
      ...
      <div className="p-4 md:p-8 space-y-6 max-w-5xl mx-auto">
        {pending.length === 0 ? (
          <div className="text-center p-12 bg-white rounded-3xl border border-gray-100 shadow-sm max-w-xl mx-auto">
            ...
            <p className="font-medium text-gray-500">Nenhuma solicitação de profissional pendente de aprovação.</p>
          </div>
        ) : (
          pending.map((w) => {
            ...
            return (
              <div
                key={w.id}
                className="bg-white p-6 md:p-8 rounded-3xl shadow-md border border-gray-100 relative overflow-hidden flex flex-col md:flex-row gap-8 hover:shadow-lg transition"
              >
                <div className="absolute top-0 right-0 bg-orange-500 text-white text-xs font-extrabold px-4 py-1.5 rounded-bl-xl">
                  REVISÃO NECESSÁRIA
                </div>
                <div className="shrink-0 flex flex-col items-center gap-3">
                  <img
                    src={
                      w.foto_perfil_url ||
                      'https://ui-avatars.com/api/?name=Worker&background=ea1d2c&color=fff'
                    }
                    className="w-24 h-24 rounded-full border-4 border-gray-50 shadow-sm object-cover"
                    alt={w.nome}
                  />
                  <span className="bg-red-50 text-[#EA1D2C] px-3 py-1 rounded-lg text-xs font-extrabold uppercase">
                    {d?.categorias?.[0] || 'Geral'}
                  </span>
                </div>
                <div className="flex-1">
                  ...
                  <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 mb-6">
                    ...
                    <p className="text-sm text-blue-900 italic">"{d?.bio || 'Sem bio informada.'}"</p>
                  </div>
                  <div className="flex flex-col sm:flex-row gap-3">
                    <button
                      onClick={() => {
                        if (window.confirm('Recusar e excluir este cadastro?')) rejectWorker(w.id);
                      }}
                      className="flex-1 bg-red-50 text-red-600 font-extrabold py-3.5 rounded-xl hover:bg-red-100 transition border border-red-200"
                    >
                      Recusar
                    </button>
                    <button
                      onClick={() => {
                        if (window.confirm('Aprovar este profissional?')) approveWorker(w.id);
                      }}
                      className="flex-1 bg-green-500 text-white font-extrabold py-3.5 rounded-xl shadow-lg shadow-green-500/30 hover:bg-green-600 transition transform hover:-translate-y-1"
                    >
...
export function AdminSupport() {
  const { supportTickets, users, showToast } = useAppStore();
  ...
  const handleReply = async (ticketId) => {
    ...
    const newMsg = {
      sender: 'ADMIN',
      text: replyMsg[ticketId].trim(),
      time: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
    };

    try {
      await addMessageToSupportTicket(ticketId, newMsg, ticket?.messages || []);
  ...
  const handleCloseTicket = async (ticketId) => {
    if (window.confirm('Encerrar ticket de suporte?')) {
      try {
        await updateSupportTicketStatus(ticketId, 'CLOSED');
        setActiveTicketId(null);
        showToast('Ticket encerrado.');
      } catch {
  ...
  return (
    <div className="pb-28 animate-fade-in w-full max-w-5xl mx-auto">
      <Header
        title={activeTicketId ? 'Chat de Suporte' : 'Central de Atendimento e Suporte'}
        showBack={Boolean(activeTicketId)}
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {openTickets.map((t) => {
                ...
                return (
                  <div
                    key={t.id}
                    className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex flex-col hover:shadow-md transition"
                  >
                    <div className="flex items-center gap-3 mb-4 border-b border-gray-100 pb-4">
                      ...
                      />
                      <div>
                        <h4 className="font-extrabold text-[#1F2937]">{u?.nome || 'Usuário'}</h4>
                    ...
                    <p className="text-sm text-gray-600 line-clamp-2 italic font-medium flex-1 mb-4">
                      "{last?.text || 'Sem mensagens'}"
                    </p>
                    <button
                      onClick={() => setActiveTicketId(t.id)}
                      className="w-full bg-[#EA1D2C]/10 text-[#EA1D2C] font-extrabold py-3 rounded-xl hover:bg-[#EA1D2C] hover:text-white transition flex items-center justify-center gap-2"
                    >
                      <i className="ph-bold ph-chat-teardrop-text" /> Abrir Atendimento
                    </button>
                  </div>
                );
              })}
            ...
            {openTickets.length === 0 && (
              <div className="text-center p-10 bg-white rounded-3xl border border-gray-100 shadow-sm">
                ...
                <p className="text-gray-500 text-sm">Todos os chamados foram respondidos.</p>
              </div>
            )}
          </>
        ) : (
                ...
                <div key={t.id} className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex flex-col animate-fade-in">
                  <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-4">
                    ...
                    <button
                      ...
                      className="bg-red-50 text-red-600 px-3 py-2 rounded-xl text-xs font-bold hover:bg-red-100 transition border border-red-100"
                    >
                      <i className="ph-bold ph-x-circle" /> Encerrar Ticket
                    </button>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <input
                      type="text"
                      value={replyMsg[t.id] || ''}
                      onChange={(e) => setReplyMsg({ ...replyMsg, [t.id]: e.target.value })}
                      onKeyDown={(e) => e.key === 'Enter' && handleReply(t.id)}
                      placeholder="Responder ao usuário..."
                      className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 outline-none focus:border-[#EA1D2C]"
                    />
                    <button
                      onClick={() => handleReply(t.id)}
                      className="px-6 rounded-xl bg-[#EA1D2C] text-white font-bold hover:bg-[#c41020] transition shadow-md flex items-center gap-2"
                    >
...
export function AdminChats() {
  ...
  const activeChats = orders.filter(
    (o) => ['PENDENTE', 'ACEITO', 'EM_ANDAMENTO'].includes(o.status)
  );

  return (
    <div className="pb-28 animate-fade-in w-full max-w-5xl mx-auto">
      <Header title="Monitoramento de Chats e Negociações" />
      <div className="px-4 md:px-8 space-y-6">
        {activeChats.length === 0 ? (
          <div className="text-center p-10 bg-white rounded-3xl border border-gray-100 shadow-sm">
            <i className="ph-fill ph-chats-teardrop text-5xl text-gray-300 mb-2" />
            <h3 className="font-extrabold text-lg text-[#1F2937]">Nenhum chat ativo no momento</h3>
          </div>
        ) : (
          activeChats.map((o) => {
                ...
                <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-100">
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-500">
                      Trabalhador: <span className="text-[#EA1D2C]">{worker?.nome || '—'}</span> | Cliente:{' '}
                      <span className="text-[#EA1D2C]">{client?.nome || '—'}</span>
                    </p>
                  </div>
                  <span className="bg-orange-50 text-orange-600 px-3 py-1 rounded-lg text-xs font-bold border border-orange-100">
                    {o.status}
                  </span>
                </div>
                <div className="bg-gray-50 p-4 rounded-xl">
                  <div className="flex justify-between items-center text-sm font-bold text-[#1F2937]">
                    <span>📅 Agendamento: {o.agendamento || 'A combinar'}</span>
                    <span className="text-[#EA1D2C]">R$ {o.preco_final || 0}</span>
                  </div>
                  {o.lastMessage && (
                    <p className="text-xs text-gray-500 mt-2 italic">
                      Última interação ({o.lastMessageTime}): "{o.lastMessage}"
                    </p>
                  )}
                </div>
              </div>

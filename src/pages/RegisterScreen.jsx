// src/pages/RegisterScreen.jsx — Cadastro completo de Cliente e Prestador com React Router
...
export default function RegisterScreen() {
  ...
  const { register, showToast } = useAppStore();
  ...
  const handleSubmit = async (e) => {
    ...
    const userData = {
      ...
      telefone,
      role,
      foto_perfil_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(nome)}&background=ea1d2c&color=fff`
    };
    const specificData = { cpf, cep, uf, cidade, rua, bairro, numero };

    let success;
    if (role === 'CONTRATANTE') {
      success = await register(userData, { ...specificData, bio_curta: '' });
      setLoading(false);
      if (success) navigate('/');
    } else {
      success = await register(userData, { ...specificData, bio, categorias: [categoria] });
      setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] flex flex-col w-full animate-fade-in relative md:py-10">
      <button
        ...
        className="absolute top-4 left-4 md:top-8 md:left-8 text-gray-400 hover:text-[#1F2937] flex items-center gap-2 font-bold bg-white px-4 py-2 rounded-lg shadow-sm z-50 transition"
      >
      ...
      <div className="flex-1 p-6 max-w-3xl mx-auto w-full mt-14 md:mt-0">
        ...
        <div className="flex bg-white rounded-2xl p-2 mb-8 shadow-sm border border-gray-100 relative">
          <button
            ...
            className={`flex-1 py-3.5 rounded-xl text-sm font-extrabold transition-all z-10 ${
              role === 'CONTRATANTE' ? 'text-white' : 'text-gray-500'
            }`}
          >
            Quero Contratar
          </button>
          <button
            ...
            className={`flex-1 py-3.5 rounded-xl text-sm font-extrabold transition-all z-10 ${
              role === 'TRABALHADOR' ? 'text-white' : 'text-gray-500'
            }`}
          >
            Quero Trabalhar
          </button>
          <div
            className={`absolute top-2 bottom-2 w-[calc(50%-0.5rem)] bg-[#EA1D2C] rounded-xl transition-all duration-300 ease-out ${
              role === 'CONTRATANTE' ? 'left-2' : 'left-[calc(50%+0.25rem)]'
            }`}
          />
        </div>
          ...
          <div className="bg-orange-50 border border-orange-200 p-5 rounded-2xl mb-8 flex gap-4 items-start shadow-sm animate-fade-in">
            ...
            <div>
              ...
              <p className="text-sm text-orange-700">
                Para garantir a segurança de todos, o cadastro de profissionais passa por uma <b>avaliação da nossa equipe</b>.
              </p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <select
                    ...
                    className={inp + ' appearance-none'}
                  >
                    {['Eletricista', 'Encanador', 'Diarista', 'Pintor', 'Montador', 'TI', 'Freteiro'].map((c) => (
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Fale sobre sua experiência e diferenciais..."
                  />
          ...
          <div className="flex items-start gap-3 bg-gray-50 p-5 rounded-2xl border border-gray-200 mt-6">
            <input
              ...
              className="mt-1 w-5 h-5 accent-[#EA1D2C] cursor-pointer"
            />
            <label htmlFor="lgpd" className="text-sm text-gray-600 font-medium cursor-pointer">
              Declaro que li e concordo com os <b>Termos de Uso e Política de Privacidade</b>. Ao me cadastrar, aceito que meus
              dados informados poderão ser armazenados e exibidos na plataforma com o objetivo exclusivo de facilitar a
              conexão de serviços, em conformidade com a <b>LGPD</b>.
            </label>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#EA1D2C] hover:bg-[#c41020] disabled:opacity-50 text-white font-extrabold text-lg py-5 rounded-2xl shadow-lg shadow-red-500/30 mt-8 transition-all transform hover:-translate-y-1 flex items-center justify-center gap-2"
          >
            {loading && <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />}
            {loading ? 'Cadastrando...' : role === 'CONTRATANTE' ? 'Concluir Cadastro de Cliente' : 'Enviar Cadastro para Avaliação'}
          </button>
        </form>
      </div>
    </div>
